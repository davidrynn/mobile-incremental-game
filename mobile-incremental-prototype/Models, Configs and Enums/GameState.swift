...
extension Phase {
    static func isUnlocked(_ phase: Phase, in state: GameState) -> Bool {
        switch phase {
        case .gather:
            return true
        case .refine:
            return state.totalOreEarned >= GameBalance.PhaseThreshold.refine
        case .deliver:
            return state.totalPartsEarned >= GameBalance.PhaseThreshold.deliver
        }
    }
}

extension GameState {
    static func phase(for state: GameState) -> Phase {
    ...
    static func resolvedPhase(for state: GameState) -> Phase {
        if Phase.isUnlocked(state.currentPhase, in: state) {
            return state.currentPhase
        }
        return GameState.phase(for: state)
    }
}
...
func apply(action: ActionType, to state: GameState, hiddenState: HiddenState) -> (state: GameState, hiddenState: HiddenState) {
    ...
    switch action {
    case .primaryTap:
        let currentPhase = GameState.resolvedPhase(for: nextState)
